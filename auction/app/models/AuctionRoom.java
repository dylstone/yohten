...
public class AuctionRoom extends UntypedActor {
		...
		if ("OK".equals(result)) {
			...
			in.onMessage(new Callback<JsonNode>() {
				public void invoke(JsonNode event) {
					...
					defaultRoom.tell(new Bid(username, event.get("text")
							.asText()), null);
	...
	public void onReceive(Object message) throws Exception {
		...
		if (message instanceof Join) {
			...
			if (members.containsKey(join.username)) {
				getSender().tell("このユーザー名は既に使用されている", getSelf());
			} else {
			...
			.format(currentDate);
			// Received a Bid message
			Bid bid = (Bid) message;

			if (bid.username.equals("ロッボト")) {
				notifyAll(
						"bid",
						bid.username,
						bid.text + " [ sent at " + timestamp + " ] ");
			} else {
				notifyAll("bid", bid.username, bid.text);
			}
	...
	public void notifyAll(String kind, String user, String text) {
		for (WebSocket.Out<JsonNode> channel : members.values()) {
			...
			event.put("kind", kind);
			event.put("user", user);
			event.put("message", text +" { @Server "+ new SimpleDateFormat("HH:mm:ss.SSS").format(new Date())+" } ");
//			event.put("message", text);
	...
	public static class Bid {
		...
		public Bid(String username, String text) {
			this.username = username;
			this.text = text;
		}
